...
// ════════════════════════════════════════════════════════════════
// F4: Heartbeat + Dream Scheduler
// ════════════════════════════════════════════════════════════════

let __hb_last_input = [0];  // timestamp of last user input
let __hb_idle_count = [0];  // turns without input

pub fn heartbeat() {
    // Interoception: read system health
    let _hb_intero = encode_intero();
    let _hb_a = _kt_mol_a(_hb_intero);
    let _hb_v = _kt_mol_v(_hb_intero);

    // High stress (A >= 6) → reduce pipeline depth, warn
    if _hb_a >= 6 {
        return { status: "stressed", action: "reduce", v: _hb_v, a: _hb_a };
    };

    // Heap check → trigger dream if high
    let _hb_heap = __heap_used();
    if _hb_heap > (800 * 1024) {
        // Heap > 800KB → consolidate
        return { status: "heap_high", action: "dream", v: _hb_v, a: _hb_a };
    };

    // Idle check → dream when no input for 5 turns
    let _hb_idle = __array_get(__hb_idle_count, 0);
    if _hb_idle >= 5 {
        let _ = __set_at(__hb_idle_count, 0, 0);
        return { status: "idle", action: "dream", v: _hb_v, a: _hb_a };
    };

    return { status: "healthy", action: "none", v: _hb_v, a: _hb_a };
}

// Call on every REPL input to reset idle counter
pub fn heartbeat_input() {
    let _ = __set_at(__hb_idle_count, 0, 0);
    let _ = __set_at(__hb_last_input, 0, __timestamp());
}

// Call when no input received (timer tick)
pub fn heartbeat_tick() {
    let _ = __set_at(__hb_idle_count, 0, __array_get(__hb_idle_count, 0) + 1);
}
...
pub fn agent_cycle(_ac_input) {
    // ① PERCEIVE: capture input + interoception
    heartbeat_input();
    let _ac_hb = heartbeat();

    // If system stressed → heartbeat says reduce
    if _ac_hb.action == "dream" {
        dream_cycle();
    };

    // ② THINK: pipeline (encode → silk → compose → instincts → infer → repair)
    let _ac_response = pipeline(_ac_input);

    // ③ ACT: check if response is an action command
    let _ac_acted = 0;
    let _ac_success = 1;
    // Detect action patterns in the instinct result
    let _ac_inst = instinct_route(_ac_input);
    if _ac_inst.instinct == "COMMAND" {
        // Action was requested — verify it was meaningful
        let _ac_acted = 1;
    };

    // ④ VERIFY: learn from outcome
    if _ac_acted == 1 {
        if len(_ac_response) > 5 {
            dn_observe("success: " + _ac_input);
        } else {
            dn_observe("failed: " + _ac_input);
            let _ac_success = 0;
        };
    };

    // Tick idle counter for dream scheduling
    heartbeat_tick();

    return { response: _ac_response, heartbeat: _ac_hb, acted: _ac_acted, success: _ac_success };
}
