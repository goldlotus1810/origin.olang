...
// ════════════════════════════════════════════════════════════════
// E5: Self-model — Nox knows what it knows
// Per-tag domain strength: qr_ratio × 0.4 + silk × 0.3 + count × 0.3
// ════════════════════════════════════════════════════════════════

pub fn kt_self_model() {
    _boot_exemplars();
    _ksi_init();
    // Count facts per tag
    let _sm_tags = ["code", "question", "greeting", "emotion", "command", "fact"];
    let _sm_counts = [0, 0, 0, 0, 0, 0];
    let _sm_silk_sum = [0, 0, 0, 0, 0, 0];
    let _sm_fi = 0;
    while _sm_fi < len(__kt_facts_arr) {
        let _sm_tag = "";
        if _sm_fi < len(__kt_fact_tags) { let _sm_tag = __array_get(__kt_fact_tags, _sm_fi); };
        if __type_of(_sm_tag) == "string" {
            // Find tag index
            let _sm_ti = 0;
            while _sm_ti < 6 {
                if __array_get(_sm_tags, _sm_ti) == _sm_tag {
                    let _ = __set_at(_sm_counts, _sm_ti, __array_get(_sm_counts, _sm_ti) + 1);
                    // Sum silk weights for this fact
                    let _sm_fmol = _kt_fast_mol(__array_get(__kt_facts_arr, _sm_fi));
                    let _sm_sw = 0;
                    // Sample: check silk to 3 nearest neighbors
                    let _sm_near = kt_nearby(_sm_fmol, 1);
                    if len(_sm_near) > 0 {
                        let _sm_sw = kt_silk_weight(_sm_fmol, __array_get(_sm_near, 0).mol);
                    };
                    let _ = __set_at(_sm_silk_sum, _sm_ti, __array_get(_sm_silk_sum, _sm_ti) + _sm_sw);
                };
                let _sm_ti = _sm_ti + 1;
            };
        };
    ...
    // Build result: strength per domain
    let _sm_result = [];
    let _sm_total = len(__kt_facts_arr);
    if _sm_total == 0 { let _sm_total = 1; };
    let _sm_ri = 0;
    while _sm_ri < 6 {
        let _sm_c = __array_get(_sm_counts, _sm_ri);
        let _sm_avg_silk = 0;
        if _sm_c > 0 { let _sm_avg_silk = __floor(__array_get(_sm_silk_sum, _sm_ri) / _sm_c); };
        // strength = 0.4 × count_ratio + 0.3 × silk_norm + 0.3 × min(count/50, 1.0)
        let _sm_count_ratio = __floor(_sm_c * 400 / _sm_total);
        let _sm_silk_norm = __floor(_sm_avg_silk * 300 / 1000);
        let _sm_count_norm = __floor(_sm_c * 300 / 50);
        if _sm_count_norm > 300 { let _sm_count_norm = 300; };
        let _sm_strength = _sm_count_ratio + _sm_silk_norm + _sm_count_norm;
        push(_sm_result, { domain: __array_get(_sm_tags, _sm_ri), count: _sm_c, strength: _sm_strength });
        let _sm_ri = _sm_ri + 1;
    };
    return _sm_result;
}

// Domain confidence for a query (used by Honesty instinct)
pub fn kt_domain_confidence(_kdc_text) {
    let _kdc_cls = kt_classify(_kdc_text);
    let _kdc_model = kt_self_model();
    let _kdc_mi = 0;
    while _kdc_mi < len(_kdc_model) {
        let _kdc_entry = __array_get(_kdc_model, _kdc_mi);
        if _kdc_entry.domain == _kdc_cls.type {
            return _kdc_entry.strength;
        };
        let _kdc_mi = _kdc_mi + 1;
    };
    return 0;
}
